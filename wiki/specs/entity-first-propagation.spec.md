...
`ReceivedMeta`. It is in the create template; the `AddReceivedEntityKey` upgrade
changeset this line used to name was removed at V1 (see
...
Outbox rows carry nullable `entity_key`, from the create template.
`OutboxStatus` includes `Superseded`, and relay claiming excludes superseded
rows.
...
Outbox tables include the terminal-status retention index from the create
template. The `AddOutboxRetentionIndex` upgrade changeset was removed at V1;
the caveat it carried still applies to any future one, because a changeset
applies inside a transaction and cannot use `CREATE INDEX CONCURRENTLY`, so
building an index that way blocks writes for the duration.
